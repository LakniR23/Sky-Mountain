document.addEventListener('DOMContentLoaded', function() {
  ...
  // Detect form type (add or edit) based on presence of hidden pID field
  const isEditForm = !!document.querySelector('input[name="pID"]');
  const formType = isEditForm ? 'edit' : 'add';
  ...
  const validationRules = {
    ...
    'pImage': {
      required: formType === 'add', // Required only for new properties
 ...
 function validateField(field, rules) {
  ...
  if (rules.required && !field.value.trim()) {
    showError(field, field.previousElementSibling ? field.previousElementSibling.textContent + ' is required' : 'This field is required');
    return false;
  }
